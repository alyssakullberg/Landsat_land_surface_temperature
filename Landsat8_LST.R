...
# Calculate land surface temperature (LST) using Landsat 8 Collection 1 imagery
# Download full tiff files with metadata from USGS EarthExplorer, and extract all the files of the scene into a single folder
...
est_LST <- function(path_B4, path_B5, path_B10, path_MTL, ext) {
  ...
  # Calculate LST
  lst = overlay(t2, e, fun=function(x,y){x / (1 + ((.00010895*x/.01438)*log(y)))})
...
path_B4 <- "{YOUR FILE PATH HERE}"
path_B5 <- "{YOUR FILE PATH HERE}"
path_B10 <- "{YOUR FILE PATH HERE}"
path_MTL <- "{YOUR FILE PATH HERE}"
